...
def main():
    ...
    with zipfile.ZipFile(update_zip_path, 'w', compression=zipfile.ZIP_DEFLATED) as zout:
        # Closure to write retrieved blobs to the update zip. Verifies the blob's hash before
        # writing and prints progress every 1000 entries.
        ...
        extract_regular_blobs(on_blob_retrieved, regular_plan)
...
def extract_regular_blobs(on_blob_retrieved, regular_plan):
    ...
    Entries are grouped by source zip to minimise open/close overhead.

    Args:
        on_blob_retrieved: Callback(sha2, data) invoked for each retrieved blob.
...
def extract_rpa_blobs(on_blob_retrieved, zpath, rpa_name, blobs, compression, zip_data_offset):
    """Extract blobs from a single RPA entry and emit them via callback.

    STORED RPA entries are accessed via raw byte-range seeks; compressed RPA
    entries are streamed in offset order.

    Args:
        on_blob_retrieved: Callback(sha2, data) invoked for each retrieved blob.
        zpath:             Path to the zip file containing the RPA entry.
        rpa_name:          Name of the RPA entry within the zip.
        blobs:             dict of sha2 -> (rpa_offset, size).
        compression:       Compression type string (e.g. "STORED", "DEFLATED").
        zip_data_offset:   Byte offset of the RPA entry's data within the zip file.
    """
    if compression == "STORED":
        # For STORED RPAs, skip zip decompression and read directly via byte-range seeks.
        ...
        with zipfile.ZipFile(zpath, 'r') as zf:
            with zf.open(rpa_name) as stream:
                # Read blobs from an open stream in sorted offset order. This guarantees that each
                # byte in `stream` is read at most once.
